...
import com.example.sneh.studentattendance.Activity.AloneActivity;
...
public class AloneAdapter extends RecyclerView.Adapter<AloneAdapter.MyviewHolder>
{
    ...
    public AloneAdapter(Activity context, List<Check> list) {
        this.context = context;
        this.list = list;
    }

    @Override
    public AloneAdapter.MyviewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        view= LayoutInflater.from(parent.getContext()).inflate(R.layout.absentstudent,parent,false);
        return new AloneAdapter.MyviewHolder(view);
    }

    @Override
    public void onBindViewHolder(AloneAdapter.MyviewHolder holder, final int position)
    {
