...
import com.example.sneh.studentattendance.Activity.PresentActivity;
...
public class PresentstudentAdapter extends RecyclerView.Adapter<PresentstudentAdapter.MyviewHolder>
{
    Activity context;
    List<Check> list;

    public PresentstudentAdapter(Activity context, List<Check> list) {
        this.context = context;
        this.list = list;
    }

    @Override
    public PresentstudentAdapter.MyviewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view;
        view= LayoutInflater.from(parent.getContext()).inflate(R.layout.absentstudent,parent,false);
        return new PresentstudentAdapter.MyviewHolder(view);
    }

    @Override
    public void onBindViewHolder(PresentstudentAdapter.MyviewHolder holder, final int position)
    {
        holder.textviewstudentid.setText(list.get(position).getKey());
        holder.textviewstudentname.setText(list.get(position).getName());
        Picasso.get().load(list.get(position).getImage()).placeholder(R.mipmap.ic_launcher).placeholder(R.mipmap.ic_launcher).into(holder.imageview);
